...
from frcapi import team_list, copr_stats
ui.page_opts(title="FRC Dashboard", fillable=True)
...
with ui.layout_columns(col_widths={"sm": 12, "md": 12, "lg": [4, 8]}):
    ...
    with ui.card(full_screen=True):
        with ui.card_header(class_="d-flex align-items-center gap-1"):
            "Player career"
            ui.input_select("stat", None, choices=copr_stats, selected="L1 Coral Count", width="auto")
...
def _():
    ...
    ui.update_selectize("players", choices=team_list)
